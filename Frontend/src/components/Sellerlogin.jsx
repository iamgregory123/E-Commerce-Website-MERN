...
const Sellerlogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("clicked");
      const response = await axios.post('http://localhost:3000/selllogin', { email, password });
      localStorage.setItem('token', response.data.token); // assuming response.data.token contains the token
      navigate('/selladd'); // Redirect to home page
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
    <div style={{backgroundImage: 'url("https://images.pexels.com/photos/7679452/pexels-photo-7679452.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2")',
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <Typography><h1 style={{ color: 'black' }}>Seller Login</h1></Typography>
          <input 
            type="email" 
            value={email} 
          ...
          <input 
            ...
            style={{ width: '250px', height: '30px', padding: '5px' }}
          />
          <button type="submit" style={{ cursor: 'pointer', height: '40px', width: '80px',color:'white', backgroundColor:'#110074', border:'none',borderRadius:'5px' }}>Login</button>
